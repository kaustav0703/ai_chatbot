...
type SpinnerProps = {
  className?: string;
  variant?: 'ripple' | 'terminal' | 'loader';
  text?: string;
  size?: string; // Tailwind classes like h-16 w-16
  colorClass?: string; // Tailwind class like border-red-500
};
  ...
  if (variant === 'ripple') {
    return (
      <div className={cn('relative flex justify-center items-center', className)}>
        <div
          className={cn(
            'absolute rounded-full border-4 animate-ping opacity-20',
            size || 'h-16 w-16',
            colorClass || 'border-dark-primary dark:border-light-primary'
          )}
        />
        <div
          className={cn(
            'absolute rounded-full border-4 animate-ping opacity-40 [animation-delay:0.3s]',
            size || 'h-12 w-12',
            colorClass || 'border-dark-primary dark:border-light-primary'
          )}
        />
